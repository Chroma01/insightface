...
class PickableInferenceSession(onnxruntime.InferenceSession): 
    ...
    def __getstate__(self):
        ...
        values = {
            ...
            'providers': providers,
            'provider_options': [
                dict(provider_options.get(provider, {}))
                for provider in providers
            ],
        }
        dimension_overrides = getattr(
            self, 'coreml_dimension_overrides', None
        )
...
class ModelRouter:
    ...
    def get_model(self, **kwargs):
        resolution_session_factory = kwargs.pop(
        ...
        if len(outputs)>=5:
            ...
            if static_shape_sessions is not _UNSET:
                detector_kwargs['static_shape_sessions'] = (
                    static_shape_sessions
                )
            return SCRFD(
...
def _provider_names_and_options(providers, provider_options):
    ...
    if isinstance(provider_options, Mapping):
        if all(
            ...
            and isinstance(provider_options[name], Mapping)
            for name in names
        ):
...
def _normalize_detector_input_size(value):
    ...
    if not isinstance(value, (list, tuple)) or len(value) != 2:
        raise ValueError(
            '_coreml_detector_input_size must contain width and height'
        )
    ...
    for axis, (dimension, value) in enumerate(zip(spatial, requested)):
        ...
        if fixed > 0:
            if fixed != value:
                ...
                raise ValueError(
                    f'detector has fixed {axis_name} {fixed}, '
                    f'received {value}'
                )
            continue
        symbol = str(dimension.dim_param)
...
def _managed_manifest_session(model_file, task, kwargs):
    ...
    if task == DETECTION_TASK and coreml_primary and (
        ...
        return _managed_detection_session(
            ...
            model_sha256=(
                descriptor.sha256
                if isinstance(descriptor, ModelTaskDescriptor)
                else _model_file_sha256(model_file)
            ),
        )
...
def _managed_detection_session(model_file, kwargs, *, model_sha256):
    ...
    if (
        not providers
        or _provider_name(providers[0]) != _COREML_PROVIDER
    ):
        return PickableInferenceSession(model_file, **values)
...
def _configure_image_preprocessing(
    ...
    if (
        ...
        and preprocessing != EMBEDDED_PREPROCESSING
    ):
        raise RuntimeError(
    ...
    model.input_dtype = (
        np.uint8 if input_type == 'tensor(uint8)' else np.float32
    )
    return model
...
def _explicit_model(model_file, task, metadata, **kwargs):
    if task not in MODEL_PACKAGE_TASKS:
        raise ValueError(
            'model_task must be detection, verification, or recognition'
        )
    ...
    if task == DETECTION_TASK:
        detector_kwargs = {}
        resolution_session_factory = kwargs.get(
            'resolution_session_factory'
        )
        if resolution_session_factory is not None:
            detector_kwargs['resolution_session_factory'] = (
        ...
        if 'static_shape_sessions' in kwargs:
            detector_kwargs['static_shape_sessions'] = kwargs[
                'static_shape_sessions'
            ]
        model = SCRFD(
...
def get_model(name, **kwargs):
    ...
    if resolution_session_factory is not None:
        router_kwargs['resolution_session_factory'] = (
            resolution_session_factory
        )
    if 'static_shape_sessions' in kwargs:
        router_kwargs['static_shape_sessions'] = kwargs[
            'static_shape_sessions'
        ]
