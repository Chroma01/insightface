...
Текущая версия: **0.3.0**, Linux x86_64.
...
**Новое в 0.3.0 и обновление:** Добавлены `raccoon_s` и `raccoon_l` с поддержкой
их манифестов, необязательная проверка живого лица с установкой модели через Web
и входные изображения BMP. В существующих установках можно сохранить модели,
конфигурацию и монтирования данных; проверка живого лица остаётся отключённой,
пока её не включат. Результаты API и SDK больше не содержат `model_version`.
См. [порядок обновления](docs/user-guide.ru.md#обновление-до-030).
